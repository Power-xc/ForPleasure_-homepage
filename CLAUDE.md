...
> **전체 상세 규칙:** `skills/개발/운영-프로토콜.md` 참조 (상세 아키텍처/보안/품질 기준은 이곳을 확인하라)
...
> 아직 `SPEC.md`/명세서가 없다. 홈페이지 첫 작업은 `skills/개발/기능명세-양식.md`를 복사해 **명세서.md 작성**부터 시작하는 것을 권장한다.
...
## 4. 아키텍처 및 상세 규칙 (위임)
> 코드 품질 룰, FSD 레이어, 네이밍 규칙, Git 정책, 디자인 토큰 및 에러 처리 표준은 방대하므로 **`skills/개발/운영-프로토콜.md`**에 위임한다. 코드를 짤 때는 반드시 해당 프로토콜을 따른다.
...
### 5-2. 디자인 토큰 `[PROD]`
- ForPleasure 전용 브랜드 토큰을 `브랜드-아이덴티티` 스킬로 먼저 확정한 뒤 `tailwind.config`/토큰 파일에 정의한다.
