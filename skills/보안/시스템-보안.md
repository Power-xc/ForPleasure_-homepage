<!-- 파일: skills/보안/시스템-보안.md | 스킬: System Security | 부서: 보안부 | ID: sys-sec -->

# 시스템 보안 스킬 (System Security Skill)
...
2. 보안 점검 루틴화
   └─ 배포 전 @sec deploy 필수 실행 (운영-프로토콜 Section 10 IGIT 연동)
   └─ 월 1회 @sec audit 전체 점검

3. 사고 기록
   └─ 리서치.md에 타임라인·원인·조치 내역 기록 (세션 날아가도 추적)
   └─ 같은 실수를 반복하지 않도록 운영-프로토콜 Constraints에 추가 `[IPAI]`
```
